...
@section('subheader')
    <br><br>
        <h1 class="header center green-text text-darken-3">Banca Examinadora</h1>
        <div class="row center">
          <h5 class="header col s12 light">Você pode buscar, criar, alterar e excluir Banca Examinadora</h5>
        </div>
    <br><br>
@stop
            ...
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
              <div class="input-field col s4">
                   <p>
                     ...
                     <label for="eventNameSearch_">Nome do evento</label>
            ...
            <form class="col s12" method="POST" action="{{ route('crud.banca') }}">
                ...
                <div class="row">
                    ...
                    <div class="input-field col s6">
                        ...
                        <label id="lEvent" ><i class="material-icons left">description</i>Evento</label>
                    </div>

                    <div class="input-field col s6">
                        <select id="professor1_" name="professor1">
                            <option>Escolha o 1 professor</option>
                                @foreach($professors as $professor)
                                    <option value="{{$professor->id}}">{{$professor->name}}</option>
                                @endforeach
                        </select>
                        <label id="lProfessor1"><i class="material-icons left">description</i>Primeiro Professor</label>
                    </div>
              </div>
              <div class="row">
                    <div class="input-field col s6">
                        <select id="professor2_" name="professor2">
                            <option>Escolha o 2 professor</option>
                                @foreach($professors as $professor)
                                    <option value="{{$professor->id}}">{{$professor->name}}</option>
                                @endforeach
                        </select>
                        <label id="lProfessor2"><i class="material-icons left">description</i>Segundo Professor</label>
                    </div>

                    <div class="input-field col s6">
                        <select id="professor3_" name="professor3">
                            <option>Escolha o 3 professor</option>
                                @foreach($professors as $professor)
                                    <option value="{{$professor->id}}">{{$professor->name}}</option>
                                @endforeach
                        </select>
                        <label id="lProfessor3"><i class="material-icons left">description</i>Terceiro Professor</label>
                    </div>
    ...
    <div class="row">
        ...
        <div class="card card-panel">
          ...
          @if(isset($results))
                @if($results->count() == 0)
                    <div class="card-panel red waves-effect waves-light" role="alert">
                        "Nenhuma Banca Examinadora foi encontrada."
                    </div>
                @else
        ...
        @else
            @if($boards == null || $boards->count() == 0)
                <div class="card-panel red waves-effect waves-light" role="alert"> "Nenhuma banca examinadora foi inserida ainda." </div>
            @else
                <table class="responsive-table">
                @foreach($boards as $board)
                    <?php
                        foreach ($events as $event)       
                            if($event->{'id'} == $board->id_event)    
                              $nameEvent = $event->{'name'};

                        foreach ($professors as $professor)
                            if($professor->{'id'} == $board->professor1)   
                                $professor1Name = $professor->{'name'};

                            else if($professor->{'id'} == $board->professor2)  
                                $professor2Name = $professor->{'name'};

                            else if($professor->{'id'} == $board->professor3)  
                                $professor3Name = $professor->{'name'};
                    ...
                    <tr>
                          ...
                          <td>
                              <button class="waves-effect waves-light btn"  onclick="edit('{{ $bancaString }}');" > <i class="material-icons left">info_outline</i> Editar </button>
                          </td>
                          <td>
                              <a href="#modal1" class="waves-effect waves-light btn modal-trigger"> <i class="material-icons left">delete</i> Excluir </a>
                          </td>
                    </tr>
                @endforeach
                </table>
